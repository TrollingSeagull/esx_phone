...
function GenerateUniquePhoneNumber()
  ...
  while not foundNumber do
    ...
    phoneNumber = math.random(100, 999) + math.random(100, 999)
...
AddEventHandler('esx_phone:tryOpenPhone', function()
	...
	if xPlayer ~= nil then
		if xPlayer.getInventoryItem('iphone').count >= 1 then
			print('openPhone-serv')
			TriggerClientEvent('esx_phone:openPhone', source)
		else
			sendNotification(source, 'Du har ingen telefon', 'warning', 2500)
		end
	end
end)
...
AddEventHandler('esx_phone:tryOpenPhoneCalling', function(target, channel, number)
	...
	if xPlayer ~= nil then
		if xPlayer.getInventoryItem('iphone').count >= 1 then
			...
			TriggerClientEvent('esx_phone:endCall', targetXPlayer.source, 'Kontakten är ej tillgänglig')
		end
	end
end)
...
AddEventHandler('esx_phone:startCall', function(number)
  ...
  if foundPlayer ~= false then
	...
	TriggerClientEvent('esx_phone:endCall', _source, 'Kontakten är ej ansluten')
  end
...
AddEventHandler('esx_phone:removePlayerContact', function(phoneNumber)
	...
	if xPlayer ~= nil then
		...
		sendNotification(source, 'Du har tagit bort en kontakt', 'success', 2500)
	end
end)
	  ...
	  if foundNumber then
		...
		if phoneNumber == xPlayer.get('phoneNumber') then
		  ...
		  if hasAlreadyAdded then
			TriggerClientEvent('esx:showNotification', _source, 'Numret finns redan i din katalog')
		  else
			...
			MySQL.Async.execute(
			  ...
			  function(rowsChanged)
				...
				TriggerClientEvent('esx:showNotification', _source, 'Kontakt tillagd')
		...
		TriggerClientEvent('esx:showNotification', source, 'Detta nummer är ogiltigt')
	  end
...
AddEventHandler('esx_phone:billCall', function(duration)
  ...
  TriggerClientEvent('esx:showNotification', _source, 'Samtalet kostade dig ~r~' .. amount .. '~s~')
...
AddEventHandler('esx_phone:bankTransfer', function(target, amount)
  ...
  if amount > 0 and amount <= xPlayer.getAccount('bank').money then
	...
	TriggerClientEvent('esx:showNotification', _source, 'Du har tagit bort ~g~' .. amount .. '~s~ till ' .. targetXPlayer.name)
